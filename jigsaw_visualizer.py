...
def plot_head_bias_comparison(metrics):
    """
    Plot comparison of bias metrics across different heads and baseline
    """
    import matplotlib.pyplot as plt
    import numpy as np
    
    if 'divdis_heads' not in metrics or 'baseline' not in metrics:
    ...
    for i, metric in enumerate(metric_names):
        ...
        # Add baseline value
        values.append(baseline_metrics['summary'][metric])
        
        # Create bar chart with colors
        bars = axes[i].bar(
            ['Head 0', 'Head 1', 'Head 2', 'Ensemble', 'Baseline'], 
            values,
            color=['blue', 'green', 'blue', 'orange', 'red']
        )
